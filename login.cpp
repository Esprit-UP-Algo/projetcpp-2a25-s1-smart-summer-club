...
void Login::onConnectClicked()
{
    ...
    QSqlQuery q;
    q.prepare("SELECT ID_EMPLOYEE, POSTE "
              "FROM EMPLOYEE "
              "WHERE EMAIL = :email AND PASSWORD = :pass");  // <-- updated column name
    ...
    m_idEmploye = q.value("ID_EMPLOYEE").toInt();
...
void Login::onForgotPasswordClicked()
{
    ...
    QSqlQuery q;
    q.prepare("SELECT ID_EMPLOYEE FROM EMPLOYEE WHERE EMAIL = :email"); // <-- updated table
    q.bindValue(":email", email);
    ...
    QSqlQuery update;
    update.prepare("UPDATE EMPLOYEE SET PASSWORD = :pwd WHERE EMAIL = :email"); // <-- updated column
    update.bindValue(":pwd",   newPwd);
    update.bindValue(":email", email);

    if (!update.exec()) {
        QMessageBox::critical(this, "Erreur SQL", update.lastError().text());
        return;
    }

    QMessageBox::information(this, "Succès", "Le mot de passe a été modifié !");
}
